...
public class UterusSelectionAnimancer : MonoBehaviour
{
	...
	[Space]
	public AnimationClip birthClip;
	...
	public void StateSet(string value, float feed)
	{
		...
		else if (StateName == "isBirth" && birthClip != null)
		{
			_state = _animancer.Play(birthClip, feedTime);
			_state.Speed = animationSpeed;
			_state.Events(this).OnEnd = delegate
			{
				_animancer.Play(idleClip, 0.25f);
