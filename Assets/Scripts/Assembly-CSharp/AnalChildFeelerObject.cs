...
public class AnalChildFeelerObject : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (isMove)
		{
			...
			{
				calcPosition = birthPoint[birthCount].position;
			}
			else if (isExit)
			{
			...
			if (Vector3.Distance(base.transform.position, calcPosition) < 0.05f)
			{
				...
				if (isBirth && !isExit && birthCount < birthPoint.Count - 1)
				{
					birthCount++;
					return;
				}
				...
				if (isExit)
				{
					_analChildFeelerManager.ClearList(this, currentSize);
	...
	{
		isStayMove = false;
		movePosition = _analChildFeelerManager.GetMovePoint();
		currentRotation = Random.Range(0f, 360f);
		conceiveParameter = _analChildFeelerManager.conceiveParameter;
	}

	public void GetStayPosition()
	{
		stayPosition = _analChildFeelerManager.GetMovePoint();
		currentRotation = Random.Range(0f, 360f);
		conceiveParameter = _analChildFeelerManager.conceiveParameter;
	...
	public void GetBirthPosition()
	{
		...
		isStay = false;
		isStayMove = false;
		movePosition = birthPoint[birthCount].position;
