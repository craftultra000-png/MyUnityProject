...
namespace FIMSpace.FTail
{
	public class FTail_Demo_GroundMovement : MonoBehaviour
	{
		...
		{
			Physics.Raycast(base.transform.position + GetUpVector() * RaycastHeightOffset, -GetUpVector(), out var hitInfo, RaycastCheckRange + Mathf.Abs(YOffset), GroundLayerMask, QueryTriggerInteraction.Ignore);
			return hitInfo;
		}
		...
		protected virtual void HandleAnimations()
		{
			...
			if (oneAnimation)
			{
				if (animatorHaveAnimationSpeedProp)
				{
					if (inAir)
					{
						animator.LerpFloatValue("AnimationSpeed", 0f, 30f);
					}
					else
					{
						animator.LerpFloatValue("AnimationSpeed", MultiplySprintAnimation ? (ActiveSpeed / BaseSpeed) : Mathf.Min(1f, ActiveSpeed / BaseSpeed), 30f);
					}
				}
			}
