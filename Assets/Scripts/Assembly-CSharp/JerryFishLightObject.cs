...
public class JerryFishLightObject : MonoBehaviour
{
	...
	[Header("Material")]
	public SkinnedMeshRenderer jellyFishMesh;
	...
	private void Awake()
	{
		...
		jellyFishMaterial = jellyFishMesh.material;
		_worldLightManager.jellyFishMaterial.Add(jellyFishMaterial);
	...
	public void OnCollisionEnter(Collision collision)
	{
		if (coolTime < 0f && collision.gameObject.CompareTag("MainCamera"))
		{
			...
			_worldLightManager.ChangeLight();
			if (OnomatopoeiaManager.instance.useOtomanopoeia)
			{
				Vector3 position = Camera.main.transform.position + Camera.main.transform.forward * 0.1f;
				OnomatopoeiaManager.instance.SpawnOnomatopoeia(position, Camera.main.transform, "JellyLight", Camera.main.transform);
			}
		}
	}
}
