...
public class FeelerSpankingMob : MonoBehaviour
{
	...
	public GameObject spankingPaintPoint;
	...
	private void FixedUpdate()
	{
		if (isCoolTime)
		{
			...
			{
				isCoolTime = false;
				_feelerSpankingMobManager.isHit = false;
			}
		}
		if (isWhip)
		{
			...
			{
				isTouch = false;
				spankingPaintPoint.SetActive(value: false);
			...
			if (timeCurrent <= timeMax / 2f)
			{
				whipTime = Mathf.Lerp(whipMin, 1f, timeCurrent / (timeMax / 2f));
			...
			{
				whipTime = Mathf.Lerp(1f, whipMin, (timeCurrent - timeMax / 2f) / (timeMax / 2f));
