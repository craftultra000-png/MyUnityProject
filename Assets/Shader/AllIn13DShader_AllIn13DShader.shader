Shader "AllIn13DShader/AllIn13DShader" {
	Properties {
		...
		[AdvancedProperty] [Enum(Off, 0, On, 1)] _ZWrite ("Depth Write", Float) = 1
		...
		[AdvancedProperty] [Toggle(_SPHERIZE_NORMALS_ON)] _SpherizeNormals ("Spherize Normals", Float) = 0
		...
		[SingleProperty] _Color ("Color", Vector) = (1,1,1,1)
		[SingleProperty] _GeneralAlpha ("General Alpha", Range(0, 1)) = 1
		...
		[EffectProperty(ParentEffect# COLOR_RAMP, Keywords(_COLOR_RAMP_ON), AllowReset# False)] [KeywordEnum(BeforeLighting, AfterLighting)] _ColorRampLightingStage ("Stage", Float) = 0
		...
		[EffectProperty(COLOR_RAMP)] _ColorRampTiling ("Tiling", Range(0.01, 10)) = 1
		[EffectProperty(COLOR_RAMP)] _ColorRampScrollSpeed ("Scroll Speed", Float) = 0
		...
		[EffectProperty(ParentEffect# SPECULARMODEL, IncompatibleWithKws(_SHADINGMODEL_PBR), Keywords(_SPECULARMODEL_CLASSIC, _SPECULARMODEL_TOON), AllowReset# True)] _Shininess ("Shininess", Range(0.01, 25)) = 16
		...
		[EffectProperty(SPECULARMODEL, TOON, ANISOTROPICTOON)] _SpecularToonSmoothness ("Specular Toon Smoothness", Range(0, 1)) = 0
		...
		[Effect(EffectID# REFLECTIONS, GroupID# Lighting, DependentOn# LIGHTMODEL)] [KeywordEnum(None, Classic, Toon)] _Reflections ("Reflections", Float) = 0
		...
		[EffectProperty(NORMAL_MAP)] [NoScaleOffset] _NormalMap ("Normal Map", 2D) = "bump" {}
		[EffectProperty(NORMAL_MAP)] _NormalStrength ("Normal Strength", Range(0, 10)) = 1
		...
		[Effect(EffectID# CAST_SHADOWS_ON, GroupID# Lighting)] [Toggle(_CAST_SHADOWS_ON)] _CastShadowsOn ("Enable Shadows (Casted and Received)", Float) = 1
		[Effect(EffectID# SCROLL_TEXTURE, GroupID# UVEffects)] [Toggle(_SCROLL_TEXTURE_ON)] _ScrollTextureOn ("Scroll Texture", Float) = 0
		[EffectProperty(SCROLL_TEXTURE)] _ScrollTextureX ("Scroll X", Float) = 1
		[EffectProperty(SCROLL_TEXTURE)] _ScrollTextureY ("Scroll Y", Float) = 1
		[Effect(EffectID# SCREEN_SPACE_UV, GroupID# UVEffects, IncompatibleWith# TRIPLANAR_MAPPING)] [Toggle(_SCREEN_SPACE_UV_ON)] _ScreenSpaceUVOn ("Screen Space UV", Float) = 0
		[EffectProperty(SCREEN_SPACE_UV)] _ScaleWithCameraDistance ("Scale with camera distance", Range(0, 1)) = 0
		...
		[EffectProperty(STOCHASTIC_SAMPLING)] _StochasticSkew ("Grid Skew", Range(0, 3)) = 0.57735026
		...
		[EffectProperty(TRIPLANAR_MAPPING)] [KeywordEnum(Local, World)] _TriplanarNormalSpace ("UV Space", Float) = 1
		...
		[EffectProperty(TRIPLANAR_MAPPING)] _TopNormalStrength ("Top Normal Map Strength", Range(0, 10)) = 1
		[EffectProperty(TRIPLANAR_MAPPING)] _FaceDownCutoff ("Face Down Cutoff", Range(-1, 1)) = 0.25
		[EffectProperty(TRIPLANAR_MAPPING)] _TriplanarSharpness ("Sharpness", Range(1, 64)) = 15
		...
		[EffectProperty(AOMAP)] _AOMapStrength ("AO Strength", Range(0, 1)) = 1
		[EffectProperty(AOMAP)] _AOContrast ("AO Contrast", Range(0, 1)) = 1
		[EffectProperty(AOMAP)] _AOColor ("AO Color", Vector) = (0,0,0,0)
		[Effect(EffectID# HIGHLIGHTS, GroupID# ColorEffects)] [Toggle(_HIGHLIGHTS_ON)] _Highlights ("Highlights", Float) = 0
		[EffectProperty(HIGHLIGHTS)] [HDR] _HighlightsColor ("Highlights Color", Vector) = (2,2,2,1)
		...
		[EffectProperty(ParentEffect# RIM_LIGHTING, Keywords(_RIM_LIGHTING_ON), AllowReset# False)] [KeywordEnum(BeforeLighting, BeforeLightingLast, AfterLighting)] _RimLightingStage ("Stage", Float) = 0
		[EffectProperty(RIM_LIGHTING)] [HDR] _RimColor ("Rim Color", Vector) = (1,1,1,1)
		[EffectProperty(RIM_LIGHTING)] _RimAttenuation ("Rim Attenuation", Range(0, 1)) = 1
		[EffectProperty(RIM_LIGHTING)] _MinRim ("Min Rim", Range(0, 1)) = 0
		[EffectProperty(RIM_LIGHTING)] _MaxRim ("Max Rim", Range(0, 1)) = 1
		...
		[EffectProperty(GREYSCALE)] _GreyscaleTintColor ("Greyscale Tint", Vector) = (1,1,1,1)
		...
		[EffectProperty(VERTEX_DISTORTION)] [Vector2] _VertexDistortionNoiseSpeed ("Scroll Speed", Vector) = (4,4,0,0)
		...
		[EffectProperty(EMISSION)] [HDR] _EmissionColor ("Emission Color", Vector) = (1,1,1,1)
		...
		[EffectProperty(HOLOGRAM)] [HDR] _HologramColor ("Hologram Color", Vector) = (1.25,2.8,6.8,1)
		...
		[EffectProperty(HOLOGRAM)] _HologramLineSpacing ("Hologram Line Spacing", Range(0.001, 5)) = 2
		[EffectProperty(HOLOGRAM)] _HologramLineSmoothness ("Hologram Line Smoothness", Range(0.01, 5)) = 2
		...
		[EffectProperty(MATCAP)] _MatcapIntensity ("Matcap Intensity", Range(0, 10)) = 1
		[EffectProperty(MATCAP)] _MatcapBlend ("Matcap Blend", Range(0, 1)) = 1
		[Effect(EffectID# HIT, GroupID# ColorEffects)] [Toggle(_HIT_ON)] _Hit ("Hit", Float) = 0
		[EffectProperty(HIT)] _HitColor ("Hit Color", Vector) = (1,1,1,1)
		[EffectProperty(HIT)] _HitGlow ("Hit Glow", Range(0, 100)) = 5
		[EffectProperty(HIT)] _HitBlend ("Hit Blend", Range(0, 1)) = 1
		...
		[EffectProperty(CONTRAST_BRIGHTNESS)] _Contrast ("Contrast", Range(0, 10)) = 1
		[EffectProperty(CONTRAST_BRIGHTNESS)] _Brightness ("Brightness", Range(-1, 1)) = 0
		...
		[EffectProperty(HEIGHT_GRADIENT)] _MinGradientHeight ("Min Height", Float) = 0
		[EffectProperty(HEIGHT_GRADIENT)] _MaxGradientHeight ("Max Height", Float) = 0.75
		[EffectProperty(HEIGHT_GRADIENT)] [HDR] _GradientHeightColor01 ("Gradient Color 01", Vector) = (0.2,0.2,0.2,1)
		[EffectProperty(HEIGHT_GRADIENT)] [HDR] _GradientHeightColor02 ("Gradient Color 02", Vector) = (1,1,1,1)
		...
		[EffectProperty(INTERSECTION_GLOW)] _DepthGlowPower ("Depth Power", Float) = 25
		[EffectProperty(INTERSECTION_GLOW)] _DepthGlowColor ("Depth Glow Color", Vector) = (1,0.987,0.6,1)
		[EffectProperty(INTERSECTION_GLOW)] _DepthGlowColorIntensity ("Color Intensity", Float) = 25
		[EffectProperty(INTERSECTION_GLOW)] _DepthGlowGlobalIntensity ("Global Intensity", Float) = 2
		...
		[EffectProperty(ALBEDO_VERTEX_COLOR)] _VertexColorBlending ("Blending", Range(0, 1)) = 1
		...
		[EffectProperty(TEXTURE_BLENDING)] [NoScaleOffset] _BlendingNormalMapG ("Blending Normal Map (G)", 2D) = "bump" {}
		[EffectProperty(TEXTURE_BLENDING)] [NoScaleOffset] _BlendingNormalMapB ("Blending Normal Map (B)", 2D) = "bump" {}
		[EffectProperty(TEXTURE_BLENDING)] [NoScaleOffset] _BlendingNormalMapWhite ("Blending Normal Map (White)", 2D) = "bump" {}
		[Effect(EffectID# DEPTH_COLORING, GroupID# ColorEffects)] [Toggle(_DEPTH_COLORING_ON)] _DepthColoringOn ("Depth Coloring", Float) = 0
		[Effect(EffectID# SUBSURFACE_SCATTERING, GroupID# ColorEffects)] [Toggle(_SUBSURFACE_SCATTERING_ON)] _SubsurfaceScattering ("Fake Subsurface Scattering", Float) = 0
		[EffectProperty(SUBSURFACE_SCATTERING)] _NormalInfluence ("Normal Influence", Range(0, 2.5)) = 0.5
		[EffectProperty(SUBSURFACE_SCATTERING)] _SSSPower ("SSS Power", Range(0.01, 20)) = 1
		[EffectProperty(SUBSURFACE_SCATTERING)] _SSSFrontPower ("SSS Front Power", Range(0.2, 20)) = 3
		[EffectProperty(SUBSURFACE_SCATTERING)] _SSSFrontAtten ("SSS Front Atten", Range(0, 1)) = 0.3
		[EffectProperty(SUBSURFACE_SCATTERING)] _SSSAtten ("SSS General Atten", Range(0, 1)) = 1
		[EffectProperty(SUBSURFACE_SCATTERING)] [HDR] _SSSColor ("SSS Color", Vector) = (1,1,1,1)
		[EffectProperty(SUBSURFACE_SCATTERING)] _SSSMap ("SSS Map", 2D) = "white" {}
		[Effect(EffectID# ALPHA_CUTOFF, GroupID# AlphaEffects)] [Toggle(_ALPHA_CUTOFF_ON)] _AlphaCutoffOn ("Alpha Cutoff", Float) = 1
		...
		[EffectProperty(FADE)] _FadeAmount ("Fade Amount", Range(0, 1)) = 0
		[EffectProperty(FADE)] _FadePower ("Fade Power", Range(0.25, 4)) = 1
		[EffectProperty(FADE)] _FadeTransition ("Fade Transition", Range(0, 0.4)) = 0.2
		[EffectProperty(FADE)] [Toggle(_FADE_BURN_ON)] _FadeBurnOn ("Use Fade Burn Color?", Float) = 0
		[EffectProperty(ParentEffect# FADE, Keywords(_FADE_BURN_ON), AllowReset# True)] [HDR] _FadeBurnColor ("Fade Burn Color", Vector) = (1,1,0,1)
		[EffectProperty(ParentEffect# FADE, Keywords(_FADE_BURN_ON), AllowReset# True)] _FadeBurnWidth ("Fade Burn Width", Range(0, 0.2)) = 0.01
		[Effect(EffectID# INTERSECTION_FADE, GroupID# AlphaEffects)] [Toggle(_INTERSECTION_FADE_ON)] _IntersectionFadeOn ("Intersection Fade", Float) = 0
		[EffectProperty(INTERSECTION_FADE)] _IntersectionFadeFactor ("Intersection Fade Factor", Range(0.1, 3)) = 1
